...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    notes TEXT,
    -- campi per integrazioni esterne (es. Booking iCal)
    external_source TEXT,                   -- es. 'booking_com_ical'
    external_uid TEXT,                      -- UID dell'evento ICS per deduplica
...
def init_db():
    with closing(get_conn()) as conn, conn:
        ...
        for q in CREATE_INDEXES:
            conn.execute(q)
        # tabella per salvare gli URL iCal per stanza
        conn.execute(
            """
...
# ---------------------- INTEGRATION: BOOKING.COM iCal ----------------------
# Per usare questa integrazione: recupera l'URL iCal della stanza da Booking.com
# (Calendario > Sincronizza calendario > Ottieni link iCal) e inseriscilo nella
# sezione laterale "Collega Booking.com (iCal)".
...
def upsert_ical_endpoint(room: str, url: str):
    with closing(get_conn()) as conn, conn:
        conn.execute(
            "INSERT INTO ical_endpoints(room, url) VALUES(?, ?) ON CONFLICT(room) DO UPDATE SET url=excluded.url, updated_at=CURRENT_TIMESTAMP",
            (room, url),
        )


def get_ical_map() -> dict:
    with closing(get_conn()) as conn:
...
def parse_guest_from_summary(summary: str) -> str:
    # Tentativo semplice di estrarre il nome ospite dal SUMMARY dell'evento ICS
    if not summary:
...
def import_ics_for_room(room: str, url: str) -> int:
    """Scarica e importa eventi da un feed iCal per una stanza.
    Deduplica tramite (external_source, external_uid).
    Ritorna il numero di prenotazioni create/aggiornate.
    """
    try:
    ...
    with closing(get_conn()) as conn, conn:
        for ev in cal.events:
            uid = getattr(ev, 'uid', None) or f"{room}-{ev.begin}-{ev.end}"
            start = ev.begin.date() if hasattr(ev, 'begin') else None
            end = ev.end.date() if hasattr(ev, 'end') else None
            if not (start and end):
                continue
            # Booking iCal di solito usa end = giorno di check-out
            ...
            if row:
                # opzionale: potremmo aggiornare date/note se cambiano
                continue
            conn.execute(
                """
                INSERT INTO bookings(
